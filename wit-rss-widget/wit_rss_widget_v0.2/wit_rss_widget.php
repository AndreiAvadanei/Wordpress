...
Plugin Name: WIT-RSS-Widget v0.2
...
$wit_feed_mashup = '<li style="padding-bottom: 15px;">
   ...
   <a target="_blank" href="%WIT_RSS_URL%" title="Citeste %WIT_RSS_TITLE% pe Stiri.IT">%WIT_RSS_TITLE% </a>
</li>';
...
function updateFeed($hash)
{
	...
	{
		 $link = preg_match_all("/<link>(.*?)<\/link>/",$item,$link);
...
{
	add_options_page('WIT RSS Box Settings', 'WIT RSS Box v0.2', 10, __FILE__, 'wit_settings_main');
}
...
function wit_settings_main()
{
	...
	switch($_GET['wit_action'])
	{
		...
		?>
           <p class="submit"><input type="button" onclick="window.location='<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=new';?>';" name="addnew" value="Add new widget" class="button-primary" /></p> 
        <?php
			$feeds = explode(';',trim(get_option('wit_rss_feeds')));
			$count = sizeof($feeds);
			...
			for($i = 0; $i < $count; $i++)
			{
				...
				?>
                  <tr>
                    <td><p class="submit"><input type="button" onclick="window.location = '<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=single&hash='.$feeds[$i]?>';" name="current_<?php echo $feeds[$i]; ?>" value="Edit <?php echo $feedopt['name']; ?> feed" class="button-primary" /></p></td>
                    <td><em><?php echo $feedopt['code']; ?></em></td>
                    <td><a class="button rbutton" onclick="if(confirm('Are you sure you want to remove <?=$feedopt['name']?> feed?')) window.location='<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=remove&hash='.$feeds[$i]?>';">Remove <?=$feedopt['name']?></a></td>
                  </tr>
                <?php
			}
			?>
		...
		case 'remove':
			...
			if(in_array($_GET['hash'],$rsses)) 
			{
				...
				echo '<div class="updated"><p><strong>'.__('Feed Removed. <a class="button rbutton" href="?page=wit_rss_widget_v0.2/wit_rss_widget.php">Go back</a>').'</strong></p></div>';
			}
		break;		
		case 'single':
			$rsses = explode(';',get_option('wit_rss_feeds'));
			...
			{					
				...
				{					
					...
					<script type="text/javascript">
						window.location = "<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=single&hash='.$_GET['hash'].'&update=true'?>";
					</script>
					<?php				
			...
			?>            
            <form method="post" action="<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=single&hash='.$_GET['hash']?>">
					<table class="form-table">
						<tbody>          
                        	<tr>
                    ...
                    <input type="button" name="goback" onclick="window.location = '<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php'?>';" value="Go back" class="button-primary" /></p>
                     <a class="button rbutton" onclick="if(confirm('Are you sure you want to remove <?=$feedopt['name']?> feed?')) window.location='<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=remove&hash='.$feedopt['hash']?>';">Remove <?=$feedopt['name']?></a>
		...
		case 'new':
			if(isset($_POST['submit']))
			{
				if(is_numeric($_POST['feedcount']) && $_POST['feeduri']!= '' && $_POST['feedmashup']!= '' && $_POST['feedimgdefault']!='') 
				{
                    ...
                    window.location = "<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=new&update=true'?>";
			...
			if($_GET['update'])
			{
				echo '<div class="updated"><p><strong>'.__('Settings saved. <a class="button rbutton" href="?page=wit_rss_widget_v0.2/wit_rss_widget.php">Go back</a>').'</strong></p></div>';
			}
			else
			{
				global $wit_feed_mashup;
				?>
					<form method="post" action="<?=$_SERVER['PHP_SELF'].'?page=wit_rss_widget_v0.2/wit_rss_widget.php&wit_action=new'?>">
					<table class="form-table">
						<tbody>          
                        	<tr>
                            	<th><label for="feedname">Enter your Feed name : </label></th>
							...
							<tr>
								<th><label for="feedimgdefault">Default image : </label></th>
								<td>
									<input id="feedimgdefault" type="text" name="feedimgdefault" value=""  />
									<span class="setting-description">The Default Image if an item doesn`t have any image.</span>
								</td>
							</tr>   
							<tr>
